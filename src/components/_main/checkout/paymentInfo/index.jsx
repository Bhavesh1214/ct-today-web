...
export default function PaymentInfo({ setCouponCode, setTotal }) {
  ...
  const { mutate, isLoading } = useMutation(api.applyCouponCode, {
    onSuccess: ({ data }) => {
      ...
      if (data.type === "percent") {
        ...
        toast.success(
          "Coupon code applied. You have saved " +
            fCurrency(cCurrency(data.discount)),
        );
        setDiscountPrice(discountedTotal + +shipping);
      }
    },
  ...
  return (
    <Card sx={{ mb: 2, borderRadius: "8px", boxShadow: "unset" }}>
      <CardContent sx={{ py: 2 }}>
        ...
        <Stack spacing={0} mt={1} mb={2} gap={1}>
          ...
          >
            ...
            <Typography variant="subtitle2">
              -{fCurrency(cCurrency(appliedDiscount || 0))}
            </Typography>
          </Stack>
          <Stack
            direction="row"
          ...
          >
            ...
            <Typography variant="subtitle2">
              {!shipping ? "Free" : fCurrency(cCurrency(shipping))}
            </Typography>
          </Stack>
        ...
        >
          ...
          <Typography variant="subtitle1">
            {fCurrency(cCurrency(discountPrice || total))}
          </Typography>
        </Stack>
      </CardContent>
    </Card>
